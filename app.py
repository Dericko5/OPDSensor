...
class AppController:
    def __init__(self):
        ...
        self._data:  dict = {}
        self._dtcs:  list = []
        self._history: deque = deque(maxlen=60 * OBD_POLL_HZ)   # 60 s of graph history
    ...
    def _obd_worker(self) -> None:
        ...
        _dtc_poll_counter = 10   # start at 10 so DTCs are polled on the very first cycle
    ...
    def _ui_tick(self) -> None:
        ...
        if state == "disconnected":
            ...
            if self._dtc_dismissed and current_codes == self._dtc_dismissed_set:
                self.ui.show_live(data, history, dtcs_empty=True)
            else:
                if self._dtc_dismissed and current_codes != self._dtc_dismissed_set:
            ...
            self._dtc_dismissed = False
            self.ui.show_live(data, history, dtcs_empty=dtcs_empty)
